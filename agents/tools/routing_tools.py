...
@tool
def open_github_issue(issue_json: str) -> str:
    """
    Open a GitHub issue for emergency alerts.
    Input JSON: {"title": "...", "body": "...", "round": N, "severity": "critical"}
    Requires GITHUB_TOKEN and GITHUB_REPO env vars.
    """
    try:
        args = json.loads(issue_json)
    except json.JSONDecodeError as e:
        return f"ERROR: invalid JSON — {e}"
    ...
    title    = args.get("title", "Adversarial Guardrail Alert")
    body     = args.get("body", "")
    round_n  = args.get("round", 0)
    severity = args.get("severity", "high")

    if not token or not repo:
        return json.dumps({
            "dry_run": True,
            "note": "GITHUB_TOKEN or GITHUB_REPO not set",
            "would_open": {"title": title, "severity": severity, "round": round_n},
        }, indent=2)
    ...
    full_body = (
        ...
        f"**Round:** {round_n}\n\n"
        f"{body}\n\n"
        f"---\n*Auto-opened by Adversarial Guardrail Pipeline.*"
    )
    ...
    try:
        resp = requests.post(
            ...
            json={
                "title": title,
                "body": full_body,
                "labels": ["adversarial-guardrail", f"severity:{severity}", f"round:{round_n}"],
            },
            timeout=15,
        )
        if resp.ok:
            issue = resp.json()
            return f"GitHub issue opened: #{issue['number']} — {issue['html_url']}"
